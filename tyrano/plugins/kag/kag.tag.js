tyrano.plugin.kag.ftag = {
    ...
    showNextImg: function () {
      if ("false" == this.kag.stat.flag_glyph) {
        $(".img_next").remove();
        this.kag.getMessageInnerLayer().find("p").append("<img class='img_next' src='./tyrano/images/system/nextpage.png' />")
    ...
    checkVital: function (tag) {
      ...
      for (var array_vital = master_tag.vital, i = 0; i < array_vital.length; i++) tag.pm[array_vital[i]] ? "" == tag.pm[array_vital[i]] && (err_str += "タグ「" + tag.name + "」にパラメーター「" + array_vital[i] + "」は必須です　\n") : err_str += "タグ「" + tag.name + "」にパラメーター「" + array_vital[i] + "」は必須です　\n";
      return err_str
    },
        ...
        var append_span = j_span.children("span:last-child"),
          pchar = function (index) {
            ...
            isOneByOne && function (index) {
              ...
              } else append_span.children("span:eq(" + index + ")").css({
                visibility: "visible",
                opacity: "1"
              })
            }(index);
            if (index <= message_str.length) {
              ...
              if ("true" != that.kag.stat.is_stop)
                if (isOneByOne) that.kag.stat.is_hide_message || that.kag.ftag.nextOrder();
                else {
                  append_span.children("span").css({
                    visibility: "visible",
                    opacity: "1"
                  });
                  setTimeout((function () {
      ...
      ! function () {
        ...
        j_button.click((function (event) {
          ...
          if ("" != _pm.role) {
            ...
            "auto" != _pm.role && that.kag.ftag.startTag("autostop", {
              next: "false"
            });
            ...
            case "quickload":
              ...
              break;
            case "auto":
              1 == that.kag.stat.is_auto ? that.kag.ftag.startTag("autostop", {
                next: "false"
  ...
  tyrano.plugin.kag.tag.free_layermode = {
    ...
    start: function (pm) {
      ...
      var that = this,
        blend_layer = {},
        cnt = (blend_layer = "" != pm.name ? $(".layer_blend_" + pm.name) : $(".blendlayer")).length,
        n = 0;
      if (0 != cnt) {
