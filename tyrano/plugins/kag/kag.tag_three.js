$.three_pos = function (str) {
  ...
  tyrano.plugin.kag.tag["3d_init"] = {
    ...
    start: function (pm) {
      ...
      const scene = new THREE.Scene,
        ...
        camera = new THREE[camera_mode](45, sc_width / sc_height, parseFloat(pm.near), parseFloat(pm.far));
    ...
    start: function (pm) {
      ...
      if ("gltf" == ext || "glb" == ext) {
        ...
        (new THREE.GLTFLoader).load(storage_url, (data => {
          ...
          "true" == pm.tonemap ? this.kag.tmp.three.models[pm.name].setToneMaped(!0) : this.kag.tmp.three.models[pm.name].setToneMaped(!1);
          "true" == pm.next && this.kag.ftag.nextOrder()
        }))
      } else if ("obj" == ext) {
      ...
      } else "mmd" == ext || alert("エラー：" + ext + "はサポートしていないファイル形式です")
    }
  };
  ...
  tyrano.plugin.kag.tag["3d_sprite_new"] = {
    ...
    start: function (pm) {
      var storage_url = "./data/" + ("" != pm.folder ? pm.folder : "others/3d/sprite") + "/" + pm.storage;
      const material = new THREE.SpriteMaterial({
        map: (new THREE.TextureLoader).load(storage_url),
        alphaTest: .01,
        transparent: !0
      });
      "true" == pm.tonemap ? material.toneMapped = !0 : material.toneMapped = !1;
      ...
      $("<img />").attr("src", storage_url).on("load", (e => {
        ...
        model.position.set(pos.x, pos.y, pos.z);
        model.rotation.set(rot.x, rot.y, rot.z);
        if ("" == pm.scale) model.scale.set(1 * parseInt(width), 1 * parseInt(height), 1);
        else {
      ...
      if ("" != pm.texture)
        if ("BoxGeometry" == pm.type && pm.texture.split(",").length > 1) {
          ...
          for (let i = 0; i < arr_texture.length; i++) {
            ...
            arr_material.push(new THREE.MeshStandardMaterial({
              map: texture
            }))
          }
  ...
  tyrano.plugin.kag.tag["3d_show"] = {
    ...
    start: function (pm) {
      ...
      if (0 != $.checkThreeModel(pm.name)) {
        ...
        if ("true" == pm.wait) model.fade("in", options, (() => {
          this.kag.ftag.nextOrder()
        }));
        else {
  ...
  tyrano.plugin.kag.tag["3d_hide"] = {
    ...
    start: function (pm) {
      if (0 != $.checkThreeModel(pm.name)) {
        ...
        if ("true" == pm.wait) model.fade("out", options, (_model => {
          this.kag.ftag.nextOrder();
  ...
  tyrano.plugin.kag.tag["3d_hide_all"] = {
    ...
    start: function (pm) {
      ...
      for (let key in models)
        if ("camera" != key) {
          cnt_fade++;
          if ("true" == pm.wait) models[key].fade("out", options, (_model => {
            three.scene.remove(_model);
            fin_fade++;
      ...
      if (0 != $.checkThreeModel(pm.name)) {
        ...
        for (let key in map_type) {
          ...
          this.kag.tmp.three.models[pm.name].toAnim(type, pos, options, (() => {
            ++cnt_fin >= cnt_type && "true" == pm.wait && this.kag.ftag.nextOrder()
          }))
        }
  ...
  tyrano.plugin.kag.tag["3d_gyro"] = {
    ...
    start: function (pm) {
      ...
      three.renderer; {
        const GyroMonitor = device_type => {
          ...
          const orientEvent = e => {
            if (1 == first_flag) {
              ...
              "rotation" == pm.mode ? three.stat.gyro.mode = 1 : three.stat.gyro.mode = 2;
          ...
          if ("pc" == device_type) {
            ...
            $(".tyrano_base").get(0).addEventListener("mousemove", mouseMoveEvent, !0)
          } else {
        ...
        (() => {
          "pc" != $.userenv() ? DeviceMotionEvent && ("function" == typeof DeviceMotionEvent.requestPermission ? DeviceMotionEvent.requestPermission().then((permissionState => {
            "granted" === permissionState && GyroMonitor("sp")
          })).catch(console.error) : GyroMonitor("sp")) : GyroMonitor("pc")
        })()
      }
      "true" == pm.next && this.kag.ftag.nextOrder()
    }
    ...
    start: function (pm) {
      ...
      var model = this.kag.tmp.three.models[pm.name].model,
        ...
        mousedown = (three.camera, parseInt(this.kag.config.scWidth), parseInt(this.kag.config.scHeight), !1),
        button = 0,
        original_v = (new THREE.Vector3, new THREE.Vector3, new THREE.Vector3, $.setVector(model)),
        first_client_x = 0,
        first_client_y = 0,
        first_model_x = 0,
        first_model_y = 0,
      ...
      function evt_mousemove(e) {
        if (mousedown)
          if (0 == button) {
            ...
            model.rotation.y = first_model_y + .005 * hen_x;
            var hen_y = first_client_y - e.clientY;
            model.rotation.x = first_model_x + .005 * hen_y
          } else if (1 == button) {
      ...
      function evt_mouseup(e) {
        first_client_x = 0;
        first_client_y = 0;
        if (0 == button) $.orgFloor(model.rotation.x, 100), $.orgFloor(model.rotation.y, 100), model.rotation.z;
        var msg = 'pos="' + (model.position.x + "," + model.position.y + "," + model.position.z) + '" rot="' + ($.orgFloor(model.rotation.x, 100) + "," + $.orgFloor(model.rotation.y, 100) + "," + $.orgFloor(model.rotation.z, 100)) + '" scale="' + ($.orgFloor(model.scale.x, 100) + "," + $.orgFloor(model.scale.y, 100) + "," + $.orgFloor(model.scale.z, 100)) + '" ';
  ...
  tyrano.plugin.kag.tag["3d_debug"] = {
    ...
    start: function (pm) {
      var three = this.kag.tmp.three,
        ...
        prevPosition = (parseInt(this.kag.config.scWidth), parseInt(this.kag.config.scHeight), {}),
        mousedown = !1,
        button = 0,
      ...
      function evt_mousewheel(e) {
        if (e.wheelDelta < 0) {
          model.scale.x -= .01 * model.scale.x;
          model.scale.y -= .01 * model.scale.y;
          model.scale.z -= .01 * model.scale.z
        } else {
          model.scale.x += .01 * model.scale.x;
          model.scale.y += .01 * model.scale.y;
          model.scale.z += .01 * model.scale.z
      ...
      function evt_mousedown(e) {
        if (0 == e.button) button = 0;
          ...
          button = 2;
          vec.set(e.clientX / window.innerWidth * 2 - 1, -e.clientY / window.innerHeight * 2 + 1, .5);
          vec.unproject(camera);
          vec.sub(camera.position).normalize();
          var distance = 0;
          distance = camera.position.z > 0 ? -camera.position.z / vec.z : camera.position.z / vec.z;
      ...
      function evt_mousemove(e) {
        if (mousedown) {
          ...
          if (0 == button) {
            ...
            model.rotation.x += .01 * moveDistance.y;
            model.rotation.y -= .01 * moveDistance.x;
            prevPosition = {
              x: e.clientX,
              y: e.clientY
            }
      ...
      function evt_mouseup(e) {
        ...
        var msg_pos = model.position.x + "," + model.position.y + "," + model.position.z,
          msg_rot = $.orgFloor(model.rotation.x, 100) + "," + $.orgFloor(model.rotation.y, 100) + "," + $.orgFloor(model.rotation.z, 100),
          msg_scale = $.orgFloor(model.scale.x, 100) + "," + $.orgFloor(model.scale.y, 100) + "," + $.orgFloor(model.scale.z, 100),
          _pm = model_obj.pm;
        _pm.pos = msg_pos;
        _pm.rot = msg_rot;
        _pm.scale = msg_scale;
